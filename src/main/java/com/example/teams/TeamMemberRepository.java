...
public interface TeamMemberRepository extends JpaRepository<TeamMember, Long> {
    ...
    @Query("SELECT COUNT(tm) > 0 FROM TeamMember tm WHERE tm.teamId = :teamId AND tm.userId = :userId AND tm.role = 'CAPTAIN'")
    boolean isCaptain(Long teamId, Long userId);

    @Query("SELECT tm.userId FROM TeamMember tm WHERE tm.teamId = :teamId")
    java.util.List<Long> findUserIdsByTeamId(Long teamId);
}
